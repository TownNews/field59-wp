<?php
/**
 * Class to manage Field59 Video intergration admin functionality.
 */
...
if ( ! class_exists( 'Field59_Video_Admin' ) ) :
	/**
	...
	class Field59_Video_Admin {
		 ...
		 *
		 * @return null|Field59_Video_Admin
		 */
		...
		public static function init_hooks() {
			// Configuration hooks.
			add_action( 'admin_menu', array( __CLASS__, 'add_config_menu_item' ), 40 );
			add_filter( 'acf/init', array( __CLASS__, 'acf_include' ) );

			// Video publishing workflow - media overlay.
			add_filter( 'media_upload_tabs', array( __CLASS__, 'add_media_tab' ), 20 );
			add_action( 'media_upload_field59', array( __CLASS__, 'insert_media_video_iframe' ) );
			add_action( 'wp_ajax_field59-import-thumbnail', array( __CLASS__, 'ajax_import_thumbnail' ) );
			add_action( 'wp_ajax_field59-search-videos', array( __CLASS__, 'ajax_search_videos' ) );

			// Additional features through supported hooks.
			add_filter( 'field59_video_image_source_approved', array( __CLASS__, 'whitelist_image_domains' ), 10 );
			add_action( 'field59_video_after_thumb_upload', array( __CLASS__, 'apply_eml_category' ), 10, 3 );
			add_action( 'field59_video_after_thumb_dup_selected', array( __CLASS__, 'apply_eml_category' ), 10, 3 );
			add_action( 'field59_video_media_table_before', array( __CLASS__, 'page_subtitle' ), 1 );
			add_action( 'field59_video_media_table_before', array( __CLASS__, 'filter_form_start' ), 10 );
			add_action( 'field59_video_media_table_before', array( __CLASS__, 'search_box' ), 20 );
			add_action( 'field59_video_media_table_before', array( __CLASS__, 'display_pagination' ), 30, 1 );
			add_action( 'field59_video_media_table_after', array( __CLASS__, 'filter_form_end' ), 10 );
			add_action( 'field59_video_media_table_after', array( __CLASS__, 'display_pagination' ), 30, 1 );
			add_action( 'field59_video_media_table_after', array( __CLASS__, 'inline_embed_js_template' ), 50 );

			// Filters to support approximating total & total pages because neither is passed from the API and must be calculated/infered.
			add_filter( 'field59_pagination_total', array( __CLASS__, 'approximate_total_count' ), 10, 3 );
			add_filter( 'field59_pagination_total_pages', array( __CLASS__, 'approximate_total_pages' ), 10, 3 );

			// Video publishing workflow - shortcode visualization.
			add_filter( 'mce_external_plugins', array( __CLASS__, 'register_tinymce_javascript' ) );
			add_action( 'print_media_templates', array( __CLASS__, 'video_shortcode_template' ) );
			add_action( 'admin_init', array( __CLASS__, 'editor_styles' ) );

			add_action( 'save_post', array( __CLASS__, 'video_embed_post_save' ), 11, 3 );
		}

		/**
		 * Sets the article to have the correct meta fields to feature the
		 * first Field59 video in the post as the hero video in the Quixa app.
		 *
		 * @param int     $post_id The ID of the active post.
		 * @param WP_Post $post    The current post.
		 * @return void
		 */
		public static function video_embed_post_save( $post_id, $post ) {
			/**
			 * Filters what post types can support video functionality.
			 *
			 * @param array $post_types List of post types by name.
			 */
			$supported_post_types = apply_filters( 'field59_supported_video_post_types', array( 'post', 'page' ) );

			// Do not run on revisions or unsupported post types.
			if (
				wp_is_post_revision( $post )
				|| ! in_array( $post->post_type, $supported_post_types, true )
				|| ! has_shortcode( $post->post_content, 'field59_video' )
			) {
				return;
			}
			...
			if ( preg_match_all( '/' . get_shortcode_regex() . '/s', $post->post_content, $matches, PREG_SET_ORDER ) ) {
				foreach ( $matches as $shortcode ) {
					...
					if ( 'field59_video' === $shortcode[2] ) {
						...
						$embed_code = Field59_Shortcodes::field59_video_shortcode( $shortcode_attrs );

						if ( ! empty( $embed_code ) ) {
							update_post_meta( $post_id, 'video_type', 'field59_video' );
							update_post_meta( $post_id, 'embed_code', $embed_code );
							PostContentTerm::add_post_content_term_video( $post_id );
						} else {
							$video_type = get_post_meta( $post_id, 'video_type' );
							/**
							 * Only mark the article as not having a video IF the previous stored type
							 * was field59 OR if there was no video type defined.
							 * This inproves compatibility with the values stored by Rayos/JW Player videos.
							 */
							if (
								empty( $video_type )
								|| 'field59_video' === $video_type
							) {
								PostContentTerm::remove_post_content_term_video( $post_id );
								if ( delete_post_meta( $post_id, 'video_type', 'field59_video' ) ) {
									// Only delete the embed code meta if the article was previously set with type field_59.
									delete_post_meta( $post_id, 'embed_code' );
								}
							}
						}
						break;
		...
		/**
		 * Adds UPP Settings Menu to the Rayos Config WordPress menu item.
		 *
		 * @return void
		 */
		...
		/**
		 * Load ACF fields form JSON file.
		 *
		 * @return void
		 */
		public static function acf_include() {
			$load_json = json_decode( file_get_contents( plugin_dir_path( __FILE__ ) . '/acf-json/field59-video-acf.json' ), true );

			foreach ( $load_json as $field_group ) {

				/**
				 * Filters the JSON field group before loading it into ACF.
				 *
				 * @param array $field_group An ACF field group array object.
				 */
				$json = apply_filters( 'field59_video_field_group_filter', $field_group );
				acf_add_local_field_group( $json );
			}
		}

		/**
		 * Adds Field59 option to set of media upload tabs.
		 *
		 * @param array $tabs List of media upload tab options.
		 * @return array
		 */
		...
		public static function insert_media_video_iframe() {
			...
			// Scripts.
			wp_register_script( 'field59-inline-embed-video', plugins_url( 'scripts/inline-embed-video.js', __FILE__ ), array( 'jquery' ), FIELD59_VIDEO_VERSION );
			...
			wp_enqueue_script( 'field59-search-videos', plugins_url( 'scripts/media-upload-search.js', __FILE__ ), array( 'jquery' ), FIELD59_VIDEO_VERSION );

			// Styles.
			wp_enqueue_style( 'field59-video-admin-media', plugins_url( 'styles/admin-media.css', __FILE__ ), FIELD59_VIDEO_VERSION );
		...
		public static function field59_draw_media_video_page() {
			...
			$response = self::make_field59_call( 'https://api.field59.com/v2/search', 'GET', $search_params );
		...
		public static function display_pagination( $pagination ) {
			...
			$table = Field59_List_Table::get_instance();
		...
		public static function editor_styles() {
			add_editor_style( plugins_url( 'styles/admin-editor.css?' . FIELD59_VIDEO_VERSION, __FILE__ ) );
		...
		public static function register_tinymce_javascript( $plugin_array ) {
			$plugin_array['field59_video'] = plugins_url( 'scripts/editor-visual-shortcodes.js?' . FIELD59_VIDEO_VERSION, __FILE__ );
			return $plugin_array;
		}
		...
		public function ajax_search_videos() {
			...
			if ( $video_count ) {
				...
				if ( ! empty( $search_term ) ) {
					...
					$tbody = sprintf( '<tr><td colspan="5">%s</td></tr>', esc_html__( 'There are no videos to display.', 'field59-video' ) );
				}
			}
	...
	Field59_Video_Admin::get_instance();
endif;
